...
public class DataHolder implements IGUITableDataCollection<DataEntry> {
   ...
   * @throws InvalidFatalCaseException
  ...
  public static DataHolder sampleData() throws Exception {
    ...
    return new DataHolder(
        Stream.of(new DataEntry(LocalDate.of(2020, Month.APRIL, 17), wi, dane, 359, 17),
            new DataEntry(LocalDate.of(2020, Month.APRIL, 18), wi, dane, 360, 17),
            new DataEntry(LocalDate.of(2020, Month.APRIL, 19), wi, dane, 361, 19)));
  }

  /**
   * Filter the data using the given {@code condition} and return a {@code DataHolder} which
   * contains the filtered data.
   *
   * @param condition - condition to be used to filter the data
   * @return {@code DataHolder} containing the filtered data
   */
